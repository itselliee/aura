...
namespace aura_core {
    void scene_renderer::on_attach() {
        create_framebuffer();
        std::cout << "ogl_render: initialized successfully" << std::endl;
    }

    void scene_renderer::create_framebuffer() {
    ...
    void scene_renderer::gl_render_stg() {
        glBindFramebuffer(GL_FRAMEBUFFER, m_framebuffer);
    ...
    void scene_renderer::on_detach() {
        cleanup_opengl();
    }

    void scene_renderer::cleanup_opengl() {
        if (SDL_GL_GetCurrentContext() == nullptr) {
            std::cout << "CRITICAL WARN: Context already destroyed! Skipping glDelete calls to avoid SIGSEGV." <<
                    std::endl;
            return;
        }
        if (m_framebuffer != 0) {
